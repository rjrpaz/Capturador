...
*/
								...
								* otro proceso, y reinicializa variables.   *
								\*******************************************/ 
								strcpy(codigo, llamante);
/*
								sprintf(cadena, "%s|%s|%s|%s|%s|%s", codigo, llamante, llamado, fecha, pulsos, duracion);
*/


								sprintf(cadena, "%s -c %s -A %s -B %s -f %s -p 0 -d %s &", DBUP, codigo, llamante, llamado, fecha, duracion);
