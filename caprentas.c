...
#define DBUP "/root/dbup_rentas"

extern char *tzname[2];
long int timezone;

int hardware, flowchar[2], parser[2], pid, pid_hijo, poschar=0;
char *logfile="/var/log/capturador.log", *device="/dev/ttyS1", *hardware_control="FALSE", *baudrate="1200", linea[MAXLINE], linea_enviada[MAXLINE];
				...
				* encuentran en el registro.                 *
				...
				if (bandera != 'n')
				{
					...
					* Calcula la fecha prop. dicha *
					...
					tiempo_antes_ptr = malloc(sizeof(struct tm));

					tiempo_despues_ptr = malloc(sizeof(struct tm));

					/* 31 de diciembre */
					if (strcmp(fecha, fecha_fin) > 0)
					{
						sprintf(anio, "%d", (tm_ptr->tm_year + 1899));
						fecha[0] = anio[2];
						fecha[1] = anio[3];
						tiempo_antes_ptr->tm_year = (tm_ptr->tm_year) - 1;
						tiempo_despues_ptr->tm_year = tm_ptr->tm_year;
					}
					else
					{
						fecha[0] = anio[2];
						fecha[1] = anio[3];
						tiempo_antes_ptr->tm_year = tm_ptr->tm_year;
						tiempo_despues_ptr->tm_year = tm_ptr->tm_year;
					}
					...
					* Obtiene la duracion *
					\*********************/
					tiempo_antes_ptr->tm_mon = (linea[20] - 48)* 10 + (linea[21] - 48) - 1;
					tiempo_despues_ptr->tm_mon = (linea[30] - 48)* 10 + (linea[31] - 48) - 1;

					tiempo_antes_ptr->tm_mday = (linea[22] - 48)* 10 + (linea[23] - 48);
					tiempo_despues_ptr->tm_mday = (linea[32] - 48)* 10 + (linea[33] - 48);

					tiempo_antes_ptr->tm_hour = (linea[24] - 48)* 10 + (linea[25] - 48);
					tiempo_despues_ptr->tm_hour = (linea[34] - 48)* 10 + (linea[35] - 48);

					tiempo_antes_ptr->tm_min = (linea[26] - 48)* 10 + (linea[27] - 48);
					tiempo_despues_ptr->tm_min = (linea[36] - 48)* 10 + (linea[37] - 48);

					tiempo_antes_ptr->tm_sec = (linea[28] - 48)* 10 + (linea[29] - 48);
					tiempo_despues_ptr->tm_sec = (linea[38] - 48)* 10 + (linea[39] - 48);
					tiempo_antes_ptr->tm_wday = 0;
					tiempo_despues_ptr->tm_wday = 0;
					tiempo_antes_ptr->tm_yday = 0;
					tiempo_despues_ptr->tm_yday = 0;
					tiempo_antes_ptr->tm_isdst = 0;
					tiempo_despues_ptr->tm_isdst = 0;
/*
					printf("Timestamp: %d,%d,%d,%d,%d,%d\n", tiempo_despues_ptr.tm_year, tiempo_despues_ptr.tm_mon, tiempo_despues_ptr.tm_mday, tiempo_despues_ptr.tm_hour, tiempo_despues_ptr.tm_min, tiempo_despues_ptr.tm_sec);
*/

					tiempo_despues = (time_t *)malloc (sizeof(time_t));

					if ((*tiempo_despues = mktime(tiempo_despues_ptr)) == (time_t)-1)
					{
						error_log ("Error usando mktime", linea);
					}

/*
					printf("Tiempo despues: %s %ld\n", asctime(&tiempo_despues_ptr), tiempo_despues);
*/
					tiempo_antes = (time_t *)malloc (sizeof(time_t));

					if ((*tiempo_antes = mktime(tiempo_antes_ptr)) == (time_t)-1)
					{
						error_log ("Error usando mktime", linea);
					}

					free (tiempo_antes_ptr);
					free (tiempo_despues_ptr);

					duracion = difftime(*tiempo_despues, *tiempo_antes);
					free (tiempo_antes);
					free (tiempo_despues);
