...
#define DBUSER "caprentas"
#define DBPASS "chirolita"
#define DBTABLE "Rentas"

#define DBUP "/root/Works/Gobierno/Capturador/dbup_rentas"
		...
		if (S_ISREG(statbuf.st_mode))
		{
			...
			* Solo procesa los archivos que puede abrir *
			...
			else
			{
				...
				{
					if (sscanf (buffer, "%[^|]|%[^|]|%[^|]|%[^|]|%[^\n]", llamante, llamado, fecha, pulsos, duracion) != 5)
					{
						perror("sscanf");
					}
					else
					{
						sprintf (command, "%s -A %s -B %s -f %s -p %s -d %s &", DBUP, llamante, llamado, fecha, pulsos, duracion);
						system(command);
						sleep(5);
					}
				} /* Fin del while */
