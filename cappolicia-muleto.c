	...
	else if (pid == 0) /* Proceso Hijo */
	{
		...
		else if (pid_hijo == 0) /* proceso hijo en el hijo */
		{
			...
			while ((read(parser[0],linea,MAXLINE)) > 0)
			{
				...
				* llamadas a los numeros gratuitos de Internet) *
				...
				* 	 (R)  : Intrusion                           *
				...
				if ((linea[23] == ' ') || ((linea[23] == 'D') && (linea[24] == 'O')) || ((linea[23] == 'D') && (linea[24] == 'X')) || ((linea[23] == 'V') && (linea[24] == 'O')) || (linea[23] == 'L') || (linea[23] == 'T') || (linea[23] == 'X') || (linea[23] == 'M'))
				{
					tarifar_saliente = TRUE;
				}
				else if ((linea[23] == 'I') || ((linea[23] == 'D') && (linea[24] == 'I')) || ((linea[23] == 'V') && (linea[24] == 'I')) || ((linea[23] == 'N') && (linea[24] == 'I')))
				{
				...
				{
					/***********************************************\ 
					* Obtiene los pulsos para ver si es entrente    *
					* o saliente.                                   *
					\***********************************************/
					j=0;
					for (i=0; i<4; i++)
				...
				* Obtiene el numero llamante.                   *
				...
				if (bandera != 'n')
				{
					...
					* entonces hay que sacar el 1er. digito del numero                  *
					*                                                                   *
					...
					* 5 - Asistida por Operadora.                                       *
					\*******************************************************************/
					if (((strcmp(llamado, "0101")) == 0) && ((linea[58] == '1') || (linea[58] == '4') || (linea[58] == '5')))
					{
						k = 59;
						l = 16;
						if (linea[58] == '1')
				...
				* Calcula la fecha prop. dicha *
				...
				if (bandera != 'n')
				{
					...
					* otro proceso, y reinicializa variables.   *
					...
					{
						sprintf(cadena, "%s -t entrante -c %s -A %s -B %s -f %s -d %s -C %s &", cadena, codigo, llamante, llamado, fecha, duracion, comentario);
						printf ("%s\n", cadena);
/*
						system(cadena);
*/
					...
					{
						sprintf(cadena, "%s -t saliente -c %s -A %s -B %s -f %s -d %s &", cadena, codigo, llamante, llamado, fecha, duracion);
						printf ("%s\n", cadena);
/*
						system(cadena);
*/
					...
					{
						sprintf(cadena, "%s -t interna -c %s -A %s -B %s -f %s -d %s &", cadena, codigo, llamante, llamado, fecha, duracion);
						printf ("%s\n", cadena);
/*
						system(cadena);
*/
